<div>
    ...
    <div id="carte_du_jeu">
        @if ($jeu->cartes->count() == 0)
            ...
            <p class="italic text-blueapollo-700">Pour ajouter une carte, cliquez sur l'une d'entre-elles dans la liste</p>
        @else
            <h3 class="px-1 py-3 h3 text-blueapollo-900">Liste des cartes</h3>
            <div class="flex flex-col gap-1">
                @foreach ($jeu->cartes as $carte)
                    <div wire:click="detacheCarte({{ $carte->id }})" wire:key="{{ $carte->id }}" title="Cliquez pour enlever la carte">
